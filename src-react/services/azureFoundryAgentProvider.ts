...
async function foundryRequest<T>(
  ...
  if (!response.ok) {
    throw new Error(`Foundry API error (${response.status}): ${response.body}`)
  }
...
export class AzureFoundryAgentProvider implements IAgentProvider {
  ...
  private async acquireToken(): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to acquire token (${response.status}): ${response.body}`)
    }

    const tokenData = (response.body ? JSON.parse(response.body) : {}) as TokenResponse
