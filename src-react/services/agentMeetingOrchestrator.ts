...
import { createProvider, buildProviderConfig } from '@/services/providerFactory'
...
export class AgentMeetingOrchestrator {
  ...
  private _interactionAllowlist: { enabled: boolean; names: string[] } | null = null
  ...
  async start(params: {
    ...
    interactionAllowlist?: { enabled: boolean; names: string[] }
  }): Promise<void> {
    ...
    this._interactionAllowlist = params.interactionAllowlist ?? null
  ...
  processChatMessage(msg: MeetingChatMessage): void {
    ...
    // Ignore messages authored by peer agents in the same meeting — otherwise
    // agents react to each other's posts and loop. (Captions use the same guard.)
    if (msg.senderDisplayName === this._displayName || this._ignoredSpeakers.has(msg.senderDisplayName)) {
      return
    }
  ...
  private _buildProviderConfig(config: MeetingAgentConfig): unknown {
    return buildProviderConfig(config, 'orchestrator-agent')
  }
  ...
  private async _processAggregatedCaption(aggregated: AggregatedCaption, mention: MentionResult): Promise<void> {
    ...
    if (!this._isAllowedToInteract(aggregated.speaker)) {
      this._log('info', `🚫 Ignoring caption mention from ${aggregated.speaker} (not on interaction allowlist)`)
      return
    }
    ...
    try {
      ...
      const enrichedText = (context ? `${context}[Question from ${aggregated.speaker}]\n${aggregated.text}` : aggregated.text)
        + this._languageDirective(intendedChannel)
      ...
      if (responseText) {
        ...
        // Reactive leave: if the speaker asked this agent to leave, exit after replying.
        if (this._isLeaveRequest(aggregated.text)) {
          await this._leaveMeeting('caption request')
        }
      } else {
  ...
  private async _processChatMentionAsync(
    ...
    if (!this._isAllowedToInteract(msg.senderDisplayName)) {
      this._log('info', `🚫 Ignoring chat mention from ${msg.senderDisplayName} (not on interaction allowlist)`)
      return
    }
    if (this._isProcessing) {
    ...
    try {
      ...
      // Reactive leave: if the user asked this agent to leave, exit the meeting
      // after replying (works for every provider, not just proactive auto-leave).
      if (this._isLeaveRequest(promptText)) {
        await this._leaveMeeting('chat request')
      }
    } catch (err) {
  ...
  /**
   * Response-language directive appended to the agent prompt. The agent always
   * replies in its configured spoken language (derived from the TTS voice
   * locale) for BOTH speech and chat, independent of the language used by
   * participants or the incoming message. This keeps the agent's persona/voice
   * consistent across channels.
   */
  private _languageDirective(_channel: ResponseChannel): string {
    const locale = this._voiceLocale()
    return locale
      ? `\n\n[Language] Always respond in ${locale} (the agent's configured language), regardless of the language used by participants or in the incoming message.`
      : `\n\n[Language] Respond in the agent's configured language.`
  }

  /** Derive a locale (e.g. "en-US") from the configured TTS voice name. */
  private _voiceLocale(): string | null {
    const v = this._speechConfig?.voiceName
    if (!v) return null
    const m = v.match(/^[a-z]{2}-[A-Za-z]{2,}/)
    return m ? m[0] : null
  }

  /**
   * Interaction allowlist gate. When enabled, the agent only responds to
   * participants whose display name is on the allowlist (case-insensitive).
   * When disabled or empty, everyone is allowed (default behavior). This
   * prevents the agent from replying to arbitrary participants in a meeting.
   */
  private _isAllowedToInteract(name: string | undefined | null): boolean {
    const al = this._interactionAllowlist
    if (!al || !al.enabled) return true
    const allowed = al.names.map(n => n.trim().toLowerCase()).filter(Boolean)
    if (allowed.length === 0) return true
    const candidate = (name ?? '').trim().toLowerCase()
    if (!candidate) return false
    return allowed.some(n => candidate === n || candidate.includes(n) || n.includes(candidate))
  }
  ...
  private async _sendChat(text: string): Promise<boolean> {
    ...
    try {
      // Meeting chat (ACS) and the bridge chat view render plain text, so
      // convert any Markdown (bold, headings, bullets, links, code) the agent
      // emits into clean plain text before sending.
      await svc.sendMessage(markdownToPlainText(text))
      return true
    } catch (err) {
  ...
  /**
   * Detect an explicit request for the agent to leave the meeting (pt/en).
   * Kept intentionally specific to avoid false positives on generic farewells.
   */
  private _isLeaveRequest(text: string): boolean {
    const t = (text || '').toLowerCase()
    const patterns = [
      /\bpode\s+sair\b/,
      /\bpode\s+se\s+desconectar\b/,
      /\bpode\s+ir\s+embora\b/,
      /\bsair\s+da\s+(reuni|chamada|call)/,
      /\bsaia\s+da\s+(reuni|chamada|call)/,
      /\bpode\s+encerrar\b/,
      /\byou\s+(can|may)\s+(leave|go|drop\s+off)\b/,
      /\bplease\s+leave\b/,
      /\bleave\s+the\s+(meeting|call|room)\b/,
      /\bdrop\s+off\s+the\s+call\b/,
      /\bhang\s+up\b/,
      /\bleave\s+now\b/,
    ]
  ...
  /**
   * Leave the meeting reactively (any provider), independent of proactive
   * auto-leave. Ends the session and asks the host to hang up the call.
   */
  private async _leaveMeeting(reason: string): Promise<void> {
    this._log('info', `🚪 Leave requested (${reason}) — exiting meeting`)
    this._stopProactiveMode()
