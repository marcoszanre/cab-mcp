  ...
  constructor(params: {
    ...
    this._orchestrator.on((event) => {
      if (event.type === 'processing-changed' && event.data) {
        ...
        } else if (!this._ttsService.isSpeaking()) {
          // Processing finished and the agent is NOT speaking — e.g. a chat-only
          // response, a proactive [NO_ACTION]/timeout, or an error. Return the
          // video to idle so the "thinking" motion doesn't get stuck forever.
          // For the speech path, TTS is already active here (isSpeaking === true)
          // and its speaking-finished callback owns the transition back to idle.
          this._acsService.setVideoState('idle')
        }
      }
    })
