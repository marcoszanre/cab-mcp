...
async function storeCopilotAuthCacheLocal(serialized: string): Promise<void> {
  await secureSet(COPILOT_AUTH_CACHE_KEY, serialized)
}

async function getCopilotAuthCacheLocal(): Promise<string | null> {
  return secureGet(COPILOT_AUTH_CACHE_KEY)
}

async function deleteCopilotAuthCacheLocal(): Promise<void> {
...
export class CopilotStudioAgentProvider implements IAgentProvider {
  ...
  private async cacheAuth(): Promise<void> {
    ...
    await storeCopilotAuthCacheLocal(serialized)
  }
  ...
  private async loadCachedAuth(): Promise<void> {
    try {
      const cached =
        (await getCopilotAuthCacheLocal()) ||
        getLegacyAuthCache()
