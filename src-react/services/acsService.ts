...
import type { CallAgent, Call, CallClient, TeamsCallAgent, TeamsCall } from '@azure/communication-calling'
...
type AnyCall = Call | TeamsCall
type EventfulCall = AnyCall & {
...
export class AcsCallService {
  ...
  private callAgent: CallAgent | TeamsCallAgent | null = null
  private currentCall: AnyCall | null = null
  ...
  private _identityMode: 'anonymous' | 'agent365' = 'anonymous'
   ...
   *
   * @param token        ACS Communication access token.
   * @param displayName  Guest display name (anonymous mode only).
   * @param identityMode 'anonymous' (default) joins as an ACS guest via createCallAgent;
   *                     'agent365' joins as the named Teams identity via createTeamsCallAgent
   *                     (the token must be a Teams-user token from getTokenForTeamsUser).
   */
  async initialize(
    token: string,
    displayName: string,
    identityMode: 'anonymous' | 'agent365' = 'anonymous',
  ): Promise<void> {
    loggers.acs.debug(`Initializing ACS CallClient (identity: ${identityMode})...`)
    ...
    try {
      ...
      if (identityMode === 'agent365') {
        // Teams-user token → named Teams identity. Name comes from the Teams
        // identity automatically, so displayName is NOT passed here.
        loggers.acs.debug('Creating TeamsCallAgent (Agent 365 named identity)...')
        this.callAgent = await initWithTimeout(
          this.callClient.createTeamsCallAgent(tokenCredential),
          20000,
          'TeamsCallAgent creation'
        )
      } else {
        loggers.acs.debug('Creating CallAgent...')
        this.callAgent = await initWithTimeout(
      ...
      loggers.acs.debug(`CallAgent created for: ${displayName} (identity: ${identityMode})`)
      
      // Set up call agent event handlers
      ;(this.callAgent as CallAgent).on('callsUpdated', (e) => {
        loggers.acs.debug(`Calls updated: ${e.added.length} added, ${e.removed.length} removed`)
      })

      // Auto-accept incoming calls (agent mode). Only wired for anonymous
      // CallAgent — TeamsCallAgent is join-only in this app.
      if (identityMode === 'anonymous') {
        (this.callAgent as CallAgent).on('incomingCall', async (e) => {
          loggers.acs.debug('Incoming call received!')
      ...
      loggers.acs.error('ACS initialize failed:', message)
  ...
  async joinMeeting(meetingUrl: string): Promise<void> {
    ...
    if (this.currentCall) {
      loggers.acs.debug('Already in a call or joining, ignoring duplicate join request')
      return
    }
    if (this._joinInProgress) {
      loggers.acs.debug('Join already in progress (mutex), waiting for it to complete')
  ...
  private async _doJoinMeeting(meetingUrl: string): Promise<void> {
    loggers.acs.debug(`Joining Teams meeting: ${meetingUrl}`)
    ...
    if (this._ttsAudioContext?.state === 'suspended') {
      loggers.acs.debug('Resuming TTS audio context...')
    ...
    loggers.acs.debug('Audio and video streams created')
    ...
    if (this._identityMode === 'agent365') {
      // Named Teams identity: TeamsCallAgent only supports joining meetings.
      const teamsAgent = this.callAgent as TeamsCallAgent
      this.currentCall = teamsAgent.join({ meetingLink: meetingUrl }, callOptions)
    } else if (meetingUrl.includes('teams.microsoft.com')) {
      this.currentCall = (this.callAgent as CallAgent).join({ meetingLink: meetingUrl }, callOptions)
    } else {
      this.currentCall = (this.callAgent as CallAgent).startCall(
        [{ communicationUserId: meetingUrl }],
        callOptions
      )
    }
    
    loggers.acs.debug('Call object created, setting up handlers...')
    await this.setupCallHandlers()
  ...
  private createTtsAudioStream(): MediaStream {
    ...
    loggers.acs.debug('TTS Audio stream created with context:', context.state)
    return dest.stream
  }
  ...
  private async setupCallHandlers(): Promise<void> {
    ...
    this.currentCall.on('isMutedChanged', () => {
      ...
      loggers.acs.debug('Mute state changed:', muted)
    ...
    this.currentCall.on('stateChanged', async () => {
      ...
      loggers.acs.debug('Call state changed:', state)
      this.lastKnownState = state
      this.onStateChanged?.(state)
      
      if (state === 'Connected') {
        loggers.acs.debug('Call connected, setting up captions...')
        ...
        try {
          ...
          loggers.acs.debug('Incoming audio muted — processing captions only')
        } catch (err) {
          loggers.acs.warn('Failed to mute incoming audio:', err)
        }
        ...
        loggers.acs.debug('Call disconnected/disconnecting:', state, reason)
        ...
        if (reason) {
          ...
          loggers.acs.debug(`Call end reason: code=${reason.code}, subCode=${reason.subCode}`)
        }
    ...
    this.currentCall.on('remoteParticipantsUpdated', (e) => {
      loggers.acs.debug(`Participants: +${e.added.length} -${e.removed.length}`)
  ...
  private startStateMonitoring(): void {
    ...
    this.callStateCheckInterval = setInterval(() => {
      const call = this.currentCall
      if (!call) {
        loggers.acs.debug('[StateMonitor] No current call, stopping monitor')
      ...
      if (currentState !== this.lastKnownState) {
        loggers.acs.debug(`[StateMonitor] State changed: ${this.lastKnownState} -> ${currentState}`)
        this.lastKnownState = currentState
        this.onStateChanged?.(currentState)
        
        if (currentState === 'Disconnected') {
          const reason = call.callEndReason as CallEndReason | undefined
          loggers.acs.debug('[StateMonitor] Call disconnected, triggering cleanup', reason)
          this.onCallEnded?.(reason || null)
          this.cleanup()
        }
      }
  ...
  private notifyChatThreadReady(): void {
    ...
    if (threadId && this.onChatThreadReady) {
      loggers.acs.debug('Chat thread ready:', threadId.substring(0, 30) + '...')
      this.onChatThreadReady(threadId)
    } else if (!threadId) {
      loggers.acs.debug('No chat thread ID available (not a Teams meeting?)')
    }
  }
  ...
  private async setupCaptions(): Promise<void> {
    ...
    try {
      loggers.acs.debug('Setting up closed captions...')
      ...
      if (captions.kind === 'TeamsCaptions') {
        ...
        loggers.acs.debug('TeamsCaptions feature available')
        ...
        if (!captions.isCaptionsFeatureActive) {
          ...
          loggers.acs.debug(`Captions started with spoken language: ${this._spokenLanguage}`)
        } else {
          loggers.acs.debug('Captions already active')
        }
      }
    } catch (error) {
      loggers.acs.error('Failed to setup captions:', error)
    }
  }
  ...
  async leaveCall(): Promise<void> {
    if (this.currentCall) {
      ...
      loggers.acs.debug('Call ended by user')
    }
  }
  ...
  async reset(): Promise<void> {
    loggers.acs.debug('🔄 ACS Service reset called')
    this.stopStateMonitoring()
    ...
    if (this.currentCall) {
      try {
        await this.currentCall.hangUp()
        loggers.acs.debug('🔄 Current call hung up')
      } catch (err) {
        loggers.acs.warn('ACS hangUp during reset failed', err)
      }
      this.currentCall = null
    }
    ...
    if (this.callAgent) {
      try {
        await this.callAgent.dispose()
        loggers.acs.debug('🔄 Call agent disposed')
      } catch (err) {
        loggers.acs.warn('ACS callAgent dispose during reset failed', err)
      }
      this.callAgent = null
    }
    ...
    this._isInitialized = false
    this._lastDisplayName = null
    loggers.acs.debug('🔄 ACS Service reset complete')
  }
}
