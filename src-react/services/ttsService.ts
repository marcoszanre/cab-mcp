...
class TTSPreprocessor {
  ...
  initialize(config: Pick<TTSConfig, 'openaiEndpoint' | 'openaiApiKey' | 'openaiDeployment'>): boolean {
    if (!config.openaiApiKey || !config.openaiEndpoint) {
      loggers.tts.warn('OpenAI not configured, using basic text cleanup only')
    ...
    if (!isDesktopRuntimeAvailable()) {
      loggers.tts.warn('TTS preprocessing requires the Tauri desktop runtime.')
      this.isEnabled = false
      this.openaiConfig = null
      this.openai = null
      this.openaiClientPromise = null
    ...
    this.isEnabled = true
    loggers.tts.debug('TTS Preprocessor enabled with Azure OpenAI')
    return true
  }
...
Return ONLY valid JSON, no markdown fences, no explanations.`
      ...
      loggers.tts.error('AI preprocessing failed, using basic cleanup:', error)
    }
...
class SpeechTextRewriter {
  ...
  initialize(config: Pick<TTSConfig, 'openaiEndpoint' | 'openaiApiKey' | 'openaiDeployment'>): boolean {
    ...
    if (!isDesktopRuntimeAvailable()) {
      loggers.tts.warn('[TTS] Speech rewriting requires the Tauri desktop runtime.')
      this.isEnabled = false
      this.openaiConfig = null
      this.openai = null
      this.openaiClientPromise = null
    ...
    this.deploymentName = config.openaiDeployment
    this.isEnabled = true
    loggers.tts.debug('[TTS] Speech text rewriter enabled')
    return true
  }
...
OUTPUT: Return ONLY the rewritten text. No explanations, no JSON, no markdown.`
      ...
      loggers.tts.warn('[TTS] Speech rewriting failed, using original text:', error)
    }
...
export class TextToSpeechService {
  ...
  set speechRate(rate: number) {
    ...
    loggers.tts.debug(`Speech rate set to ${this._speechRate}`)
  }
  ...
  set speakingStyle(style: string) {
    ...
    loggers.tts.debug(`Speaking style set to ${this._speakingStyle}`)
  }
  ...
  setVoiceForLanguage(lang: string): boolean {
    ...
    if (!match) {
      loggers.tts.warn(`[TTS] No voice found for language "${lang}", keeping current voice`)
      return false
    }
    this.voiceName = match.voiceName
    this._language = match.locale
    loggers.tts.debug(`[TTS] Voice switched to ${match.voiceName} for language ${match.locale}`)
    return true
  }
  ...
  async initialize(config: TTSConfig): Promise<boolean> {
    try {
      loggers.tts.debug('Initializing TTS service...')
      ...
      if (!this.speechKey) {
        loggers.tts.warn('Speech service key not configured. TTS will not work.')
        return false
      }
      ...
      loggers.tts.debug('TTS service initialized successfully')
      return true
    } catch (error) {
      loggers.tts.error('Failed to initialize TTS service:', error)
      return false
    }
  }
  ...
  private async synthesizeSpeech(text: string): Promise<ArrayBuffer> {
    ...
    loggers.tts.debug(`[TTS] Synthesizing with ${provider.name} (rate: ${this._speechRate})...`)
    ...
    loggers.tts.debug(`[TTS] Synthesized ${result.audioData.byteLength} bytes via ${provider.name}`)
  ...
  async resumeAudioContexts(): Promise<void> {
    try {
      if (this._ttsAudioContext?.state === 'suspended') {
        ...
        loggers.tts.debug('[TTS] TTS audio context pre-resumed')
      }
      if (this.audioContext?.state === 'suspended') {
        await this.audioContext.resume()
        loggers.tts.debug('[TTS] Local audio context pre-resumed')
      }
    } catch (err) {
      loggers.tts.warn('[TTS] Audio context pre-resume failed:', err)
    }
  }
  ...
  setCallMode(active: boolean): void {
    ...
    loggers.tts.debug(`[TTS] Call mode ${active ? 'ENABLED' : 'DISABLED'} — local speaker ${active ? 'silenced' : 'active'}`)
  }
  ...
  private async playAudioBuffer(audioBuffer: AudioBuffer): Promise<void> {
    ...
    if (this._ttsAudioContext && this._ttsGainNode && this._ttsDestination) {
      loggers.tts.debug('[TTS] Injecting audio into call stream via ttsAudioContext (state:', this._ttsAudioContext.state, ')')
      ...
      await new Promise<void>((resolve) => {
        ...
        source.start(0)
        loggers.tts.debug('[TTS] Audio injection started — audio routed to meeting only')
      })
    } else {
      ...
      if (inCall || this._callModeActive) {
        loggers.tts.error('[TTS] In active call but call audio context is missing (ttsAudioContext:', this._ttsAudioContext,
      ...
      loggers.tts.debug('[TTS] No active call — playing through system speakers (preview/testing mode)')
  ...
  async speakText(text: string, options?: SpeakTextOptions): Promise<string> {
    try {
      ...
      loggers.tts.debug('[TTS] Original text:', text.substring(0, 100) + '...')
      ...
      loggers.tts.debug('[TTS] Cleaned text:', preprocessResult.text.substring(0, 100) + '...')
      const spokenText = rewrittenText
      if (rewrittenText !== basicCleaned) {
        loggers.tts.debug('[TTS] Rewritten for speech:', spokenText.substring(0, 100) + '...')
      }
      ...
      if (options?.unmuteDuringPlayback && muteCallback) {
        this.onSpeakingFinished = async () => {
          try {
            ...
            loggers.tts.warn('Failed to re-mute:', error)
          }
        }
      }
      ...
      try {
        ...
        loggers.tts.debug('Trying WAV header fallback...')
      ...
      loggers.tts.error('Error in speakText:', error)
  ...
  async speakTextStreaming(text: string, options?: SpeakTextOptions): Promise<string> {
    ...
    try {
      ...
      // Step 1+2: Run preprocessing and conversational rewrite in parallel
      loggers.tts.debug('[TTS] [Streaming] Original text:', text.substring(0, 100) + '...')
      const basicCleaned = options?.preserveText ? text : this.preprocessor.basicCleanup(text)
      ...
      loggers.tts.debug('[TTS] [Streaming] Cleaned text:', preprocessResult.text.substring(0, 100) + '...')
      const spokenText = rewrittenText
      if (rewrittenText !== basicCleaned) {
        loggers.tts.debug('[TTS] [Streaming] Rewritten for speech:', spokenText.substring(0, 100) + '...')
      }
      ...
      loggers.tts.debug(`[TTS] [Streaming] Starting stream synthesis with ${provider.name}...`)
      ...
      player.onPlaybackStarted = () => {
        loggers.tts.debug('[TTS] [Streaming] Playback started — first audio chunk playing')
      }

      player.onPlaybackFinished = () => {
        loggers.tts.debug('[TTS] [Streaming] Playback finished')
        this.onSpeakingFinished?.()
      }
      ...
      if (options?.unmuteDuringPlayback && options.muteCallback) {
        try {
          ...
          loggers.tts.warn('[TTS] [Streaming] Failed to re-mute:', muteErr)
        }
      }
      ...
      this._acsService?.setSpeaking(false)
      loggers.tts.error('[TTS] [Streaming] Error:', error)
      throw error
    }
  }
  ...
  playThinkingTone(): void {
    // Disabled: the audible "thinking" chime before TTS was perceived as a
    // creepy beep. Kept as a no-op so all existing call sites stay valid; the
    // visual ring "processing" animation already signals that the agent is
    // working.
    return
  }
  ...
  stopThinkingTone(): void {
    if (this._thinkingOscillator) {
      ...
      loggers.tts.debug('[TTS] Thinking tone stopped')
    }
  }
  ...
  set rewriteEnabled(enabled: boolean) {
    ...
    loggers.tts.debug(`[TTS] Conversational rewriting ${enabled ? 'enabled' : 'disabled'}`)
  }
