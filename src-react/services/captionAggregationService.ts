 ...
 */
export class CaptionAggregationService {
  // Configuration
  private aggregationWindowMs: number = 1500 // 1.5s to aggregate captions (lower = faster response)
  private pendingMentionTimeoutMs: number = 2000 // 2s to wait for follow-up after name-only mention
  ...
  private async getOpenAIClient(): Promise<OpenAI | null> {
    if (!this.gptEnabled || !this.openaiConfig) {
      return null
    }
  ...
  initialize(agentName: string, variations?: string[]): void {
    ...
    loggers.caption.debug('📝 Caption Aggregation Service initialized')
    loggers.caption.debug('   Agent name:', this.agentName)
    loggers.caption.debug('   Agent variations:', this.agentNameVariations)
    loggers.caption.debug('   Aggregation window:', this.aggregationWindowMs, 'ms')
    loggers.caption.debug('   Pending mention timeout:', this.pendingMentionTimeoutMs, 'ms')
    loggers.caption.debug('   GPT enabled:', this.gptEnabled)
  }
  ...
  initializeGpt(config: GptConfig): boolean {
    if (!config.openaiApiKey || !config.openaiEndpoint) {
      loggers.caption.warn('OpenAI not configured for caption enhancement')
    ...
    if (!isDesktopRuntimeAvailable()) {
      loggers.caption.warn('OpenAI caption enhancement requires the Tauri desktop runtime.')
      this.gptEnabled = false
      this.openaiConfig = null
      this.openai = null
      this.openaiClientPromise = null
    ...
    loggers.caption.debug('🤖 GPT Caption Enhancement initialized')
    loggers.caption.debug('   Deployment:', this.deploymentName)
    loggers.caption.debug('   Ambiguous threshold:', this.gptAmbiguousThreshold)
    return true
  }
  ...
  private generatePhoneticVariations(): void {
    ...
    loggers.caption.debug('   Phonetic variations generated:', Object.fromEntries(this.phoneticVariations))
  }
  ...
  async detectMentionWithGpt(
    ...
    if (localResult.isMentioned && localResult.confidence >= this.gptAmbiguousThreshold) {
      loggers.caption.debug('🎯 High confidence local match, skipping GPT')
      return localResult
    }
    ...
    try {
      ...
      const result = await this.gptAnalyzeCaption(text, recentContext, shouldCheckIndirect, openai)
      
      if (result.nameDetected) {
        loggers.caption.debug('🤖 GPT detected agent reference:', result)
        return {
          isMentioned: true,
      ...
      loggers.caption.error('GPT caption analysis failed, using local result:', error)
      return localResult
    }
  }
...
${UNTRUSTED_CONTENT_GUARDRAIL}`
      ...
      const completion = await openai.chat.completions.create({
        messages: [
          ...
          { role: 'user', content: wrapUntrustedContent(text) }
        ],
      ...
      if (corrected !== text) {
        loggers.caption.debug('📝 GPT corrected caption:', { original: text, corrected })
      }
      
      return corrected
      
    } catch (error) {
      loggers.caption.error('GPT caption correction failed:', error)
      return text
    }
  }
  ...
  private async gptAnalyzeCaption(
    ...
    const openai = openaiClient ?? await this.getOpenAIClient()
    if (!openai) {
      throw new Error('OpenAI not initialized')
    }

    const contextStr = recentContext?.length 
      ? `\nRecent conversation (untrusted participant content):\n${wrapUntrustedContent(recentContext.slice(-3).join('\n'))}\n`
      : ''
...
${UNTRUSTED_CONTENT_GUARDRAIL}`
    ...
    const completion = await openai.chat.completions.create({
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: `Analyze this caption (untrusted participant content):\n${wrapUntrustedContent(text)}` }
      ],
      model: this.deploymentName,
      max_completion_tokens: 200
    })
    ...
    try {
      ...
      return {
        correctedText: typeof raw.correctedText === 'string' ? raw.correctedText : text,
        nameDetected: strictBoolean(raw.nameDetected),
        detectedAs: typeof raw.detectedAs === 'string' ? raw.detectedAs : null,
        isIndirectReference: strictBoolean(raw.isIndirectReference),
  ...
  async detectMentionHybrid(
    ...
    if (this.gptEnabled) {
      ...
      if (localResult.isMentioned && localResult.confidence >= this.gptMinConfidenceThreshold) {
        loggers.caption.debug('🔍 Medium confidence match, validating with GPT...')
        return this.detectMentionWithGpt(text, localResult, recentContext)
      }
      
      // No local match: check for indirect references with GPT
      loggers.caption.debug('🔍 No local match, checking for indirect references with GPT...')
      return this.detectMentionWithGpt(text, localResult, recentContext)
    }
  ...
  private processBuffer(currentSpeaker: string): void {
    ...
    // Create aggregated caption — use the most recent caption's language
    const lastLang = speakerCaptions[speakerCaptions.length - 1].spokenLanguage
  ...
  private setPendingMention(pending: PendingMention): void {
    ...
    loggers.caption.debug('⏳ Pending mention set, waiting for follow-up question:', pending.captionText)
    
    // Set timeout for pending mention
    this.pendingMentionTimer = setTimeout(() => {
      if (this.pendingMention) {
        loggers.caption.debug('⏰ Pending mention timeout - processing anyway')
