...
export class MeetingChatService {
  ...
  async initialize(
    ...
    try {
      loggers.chat.debug('Initializing Meeting Chat Service...')
      ...
      loggers.chat.debug('Chat client created successfully')
      return true
    } catch (error) {
      loggers.chat.error('Failed to initialize chat client:', error)
  ...
  async connectToThread(threadId: string): Promise<boolean> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (ok) {
        ...
        loggers.chat.warn(`[Chat] Connection health check failed on attempt ${attempt}/${maxAttempts}`)
        // Disconnect and retry
        await this.disconnect()
      }
      if (attempt < maxAttempts) {
        const delay = 2000 * Math.pow(2, attempt - 1) // 2s, 4s
        loggers.chat.debug(`[Chat] Retrying chat connection in ${delay}ms (attempt ${attempt + 1}/${maxAttempts})...`)
        await new Promise(r => setTimeout(r, delay))
      }
    }
    loggers.chat.error('[Chat] All connection attempts failed')
    return false
  }
  ...
  private async _connectToThreadOnce(threadId: string): Promise<boolean> {
    if (!this.chatClient) {
      loggers.chat.error('Chat client not initialized')
      return false
    }

    try {
      loggers.chat.debug(`Connecting to chat thread: ${threadId}`)
      ...
      this._chatMessageHandler = (event: unknown) => {
        ...
        loggers.chat.debug('Chat message received:', message)
      ...
      loggers.chat.debug('Connected to meeting chat thread')
      
      return true
    } catch (error) {
      loggers.chat.error('Failed to connect to chat thread:', error)
  ...
  private async _tryRegisterAsParticipant(): Promise<void> {
    ...
    try {
      ...
      loggers.chat.debug('[Chat] Successfully registered as chat participant')
    } catch (error) {
      // Expected to fail in some Teams policies — log and continue
      loggers.chat.debug('[Chat] Participant self-registration skipped (may already be added):', (error as Error)?.message || error)
    }
  }
  ...
  private async _verifyConnection(): Promise<boolean> {
    ...
    try {
      ...
      await pages.next()
      loggers.chat.debug('[Chat] Connection verified — message list succeeded')
      return true
    } catch (error) {
      loggers.chat.warn('[Chat] Connection verification failed:', error)
      return false
    }
  }
  ...
  async sendMessage(content: string): Promise<string | null> {
    if (!this.chatThreadClient) {
      loggers.chat.error('Not connected to chat thread')
      return null
    }
    ...
    try {
      ...
      loggers.chat.debug(`Message sent with id: ${result.id}`)
  ...
  async getMessages(maxMessages: number = 50): Promise<MeetingChatMessage[]> {
    ...
    try {
      ...
      loggers.chat.error('Failed to get messages:', error)
      return []
    }
  }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      loggers.chat.error('Error stopping notifications:', error)
    }
    ...
    loggers.chat.debug('Disconnected from meeting chat')
  }
