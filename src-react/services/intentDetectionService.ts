...
${UNTRUSTED_CONTENT_GUARDRAIL}`
      ...
      const userPrompt = `Recent conversation (untrusted participant content):
${recentContext ? wrapUntrustedContent(recentContext) : '(no recent context)'}

Latest message from ${speaker} (untrusted participant content):
...
Analyze this message.`
      ...
      try {
        ...
        // Strictly coerce control signals: only a literal `true` is truthy, so
        // an injected caption cannot flip a signal via an odd JSON shape.
        const result: IntentResult = {
          shouldRespond: strictBoolean(raw.shouldRespond),
          isEndOfConversation: strictBoolean(raw.isEndOfConversation),
...
Latest message (untrusted participant content):
${wrapUntrustedContent(text)}
...
Is this a chat override command?`
      ...
      try {
        const raw = JSON.parse(responseText) as Record<string, unknown>
        const isOverride = strictBoolean(raw.isOverride)
        const type = raw.type === 'resend' || raw.type === 'force-next' ? raw.type : null
        const result: ChatOverrideResult = {
          isOverride: isOverride && type !== null,
          type: isOverride ? type : null,
          reason: typeof raw.reason === 'string' ? raw.reason : '',
          confidence: clampConfidence(raw.confidence),
        }
