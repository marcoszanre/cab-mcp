...
describe('CopilotAuthService', () => {
  ...
  it('persists auth cache through secure storage (localStorage fallback in tests)', async () => {
    const service = new CopilotAuthService({
      clientId: 'client-id',
      tenantId: 'tenant-id',
      environmentId: 'environment-id',
    ...
    // The secure store write is async (fire-and-forget). Outside the desktop
    // runtime it falls back to localStorage, so the value lands there shortly.
    await vi.waitFor(() => {
      expect(localStorage.getItem('copilot-auth-cache')).toBeTruthy()
    })
