...
import {
  createProvider as createProviderShared,
  buildProviderConfig as buildProviderConfigShared,
...
export interface MeetingAgentConfig {
  type: 'copilot-studio' | 'azure-foundry' | 'coding-agent'
...
interface UseMeetingAgentReturn {
  ...
  // Actions
  connect: (config: MeetingAgentConfig) => Promise<{ success: boolean; conversationId: string | null; error?: string }>
...
export function useMeetingAgent(options?: UseMeetingAgentOptions): UseMeetingAgentReturn {
  ...
  const createProvider = useCallback(async (config: MeetingAgentConfig): Promise<IAgentProvider> => {
    return createProviderShared(config)
  }, [])
  ...
  const buildProviderConfig = useCallback((config: MeetingAgentConfig): unknown => {
    return buildProviderConfigShared(config, 'meeting-agent')
  }, [])

  /**
   * Connect to an agent
   */
  const connect = useCallback(async (config: MeetingAgentConfig): Promise<{ success: boolean; conversationId: string | null; error?: string }> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : (typeof err === 'string' ? err : 'Failed to connect')
      setError(errorMessage)
      setIsConnecting(false)
      isConnectingRef.current = false
      setConnectionState('error')
      log(`❌ Connection failed: ${errorMessage}`, 'error')
      return { success: false, conversationId: null, error: errorMessage }
    }
  }, [
    createProvider,
    buildProviderConfig,
