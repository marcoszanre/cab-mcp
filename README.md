...
- [Agent Behavior & Interaction Modes](#agent-behavior--interaction-modes)
  ...
  - [Interaction Allowlist](#interaction-allowlist)
...
- **Reactive leave** — Ask any agent to leave ("you can leave", "pode sair") in chat or captions and it exits the meeting
- **Interaction allowlist** — Optionally restrict which participants an agent responds to (by Teams display name), via chat and voice
- **Multi-agent per meeting** — Multiple agents can join the same meeting, each with independent behavior and configuration (agents ignore each other to avoid loops)
...
- **Copilot CLI (Coding Agent)** — Runs the local GitHub Copilot CLI as a meeting agent via the official `github-copilot-sdk`, with your configured MCP servers (Work IQ, etc.) available. See [Agent Types](#agent-types).
...
2. **Rust** (latest stable, ≥1.94 for the Copilot CLI SDK) — [Install via rustup](https://rustup.rs/)
3. **Visual Studio Build Tools** (Windows only)
   - Install from [Visual Studio](https://visualstudio.microsoft.com/downloads/)
   - Select "Desktop development with C++"
4. *(Optional)* **GitHub Copilot CLI** — required only for the Copilot CLI agent type; install and sign in per the [GitHub Copilot CLI docs](https://docs.github.com/copilot/how-tos/use-copilot-agents/use-copilot-cli)
...
| Copilot Studio / Azure AI Foundry / Copilot CLI | The AI agent that answers questions |
...
The file has exactly two top-level keys: **`config`** (all settings — service credentials, MCP server, meeting behavior) and **`agents`** (the configured agents).
...
```jsonc
{
  "config": {
    ...
    "meetingBehavior": {
      "autoLeave": { "enabled": true, "timeoutMinutes": 5, "warningBeforeLeaveMs": 60000 },
...
> Older config files that placed `mcpConfig` and `meetingBehavior` at the top level (and a legacy `config.copilotStudio` block) are migrated automatically on load and rewritten into the `config.mcp` / `config.meetingBehavior` layout on the next save.
...
## Agent 365 Named Identity (experimental — disabled)

> **Status: dormant.** This feature is **disabled** and hidden from the UI and config file. The
> code is retained behind the `AGENT365_FEATURE_ENABLED` flag (`src-react/types/index.ts`) as a
> foundation for future work. All agents currently join as an **anonymous guest**.

The goal was to let an agent join Teams meetings as a **named, licensed [Agent 365](https://learn.microsoft.com/en-us/entra/agent-id/) agent user** — appearing in the roster as that identity rather than an anonymous participant — via **ACS Teams-user interop** and the headless [agent-user OAuth flow](https://learn.microsoft.com/en-us/entra/agent-id/agent-user-oauth-flow) (`user_fic` on-behalf-of token chain, no interactive login).

The token/identity layer works end-to-end (delegated token → `getTokenForTeamsUser` → ACS token → join). However, higher-level meeting behaviors proved unreliable and the feature was parked:

- **Cross-tenant meetings reject the named identity** — the agent user can only join meetings hosted in **its own home tenant**; joining a meeting organized in another tenant leaves it stuck in the lobby ("Admitting…" / "action failed").
- **Mention detection and welcome messages** did not fire reliably for the named identity.

**To revive it:** set `AGENT365_FEATURE_ENABLED = true`, re-add the `config.agent365` block, and address the behaviors above. The implementation lives in `agent365TokenService.ts` (token chain) and `sessionManager.ts` (join wiring).
...
Run the local **GitHub Copilot CLI** as a meeting agent. CAB drives the installed `copilot` binary through the official Rust `github-copilot-sdk` (JSON‑RPC), keeping **one persistent Copilot session per meeting** so tool/MCP context and conversation history persist across turns.

**Authentication:** none in‑app — it uses your existing GitHub Copilot CLI sign‑in (`copilot` must be installed and logged in). CAB only checks that the binary is present and authenticated.

| Field | Description |
|-------|-------------|
| `model` | Model to use (discovered dynamically from the CLI; e.g. `claude-opus-4.8`) |
| `cwd` | Working directory the agent runs in (defaults to a scoped dir) |
| `systemMessage` | Optional system message to steer behavior |
| `allowShell` | Allow the autonomous `shell` tool. **Default: false** (XPIA guardrail — meeting captions are untrusted input) |
| `bin` | Optional explicit path to the `copilot` executable |
| `timeoutMs` | Per‑turn timeout |

**Highlights:**
- **Your MCP servers are available** — CLI config discovery is enabled, so every server in `~/.copilot/mcp-config.json` (e.g. **Work IQ** for Microsoft 365 data, Microsoft Learn, Power BI…) is usable by the agent. OAuth tokens are persisted so previously‑authorized servers don't re‑prompt.
- **Security‑gated tools** — all tools are enabled *except* `shell`, which is denied by default as a cross‑prompt‑injection (XPIA) guardrail. Toggle with `allowShell`.
- **Chat‑first** — responses are Markdown‑aware and converted to clean plain text for Teams chat/voice.
- **Pre‑warmed** — a background warmup runs on join so the first in‑meeting response is fast (MCP/model load is moved to join time). Sends are serialized so the warmup never collides with the first real question.
- **Single instance** — only one Copilot CLI agent can be configured at a time.

**Setup:**
1. Install and sign in to the [GitHub Copilot CLI](https://docs.github.com/copilot/how-tos/use-copilot-agents/use-copilot-cli) (`copilot`)
2. *(Optional)* Add MCP servers to `~/.copilot/mcp-config.json` — e.g. the [Work IQ MCP server](https://learn.microsoft.com/microsoft-365/copilot/extensibility/work-iq/mcp/overview) for Microsoft 365 data
3. In CAB → **Agents** → **Add Agent** → select **Copilot CLI**
4. Pick a model, set the working directory/system message as needed, and save
...
**Reactive leave** — Ask an agent to leave in chat or captions (e.g. "you can leave now", "pode sair da reunião") and it exits the meeting after replying — for any provider, independent of proactive auto‑leave.

**Response language** — Agents reply in their configured voice language across both chat and speech, regardless of the language spoken by participants.

**Multi‑agent safety** — When multiple agents share a meeting, each ignores the others' captions and chat messages, so a mention of one agent never makes the others react (no loops).

### Interaction Allowlist

Restrict **which participants** an agent will respond to. In **Settings → Meeting Behavior**, enable the allowlist and add the Teams **display names** of the people the agent is allowed to answer.

- When **enabled**, mentions from anyone **not** on the list are ignored — across **both** chat and voice (captions).
- When **disabled** (default), the agent responds to any participant who mentions it.
- Useful to stop an agent from replying to everyone in a live meeting, e.g. limit it to the presenter or a specific host.

It is configured per the `interactionAllowlist` setting in `cab-config.json`:

```json
"interactionAllowlist": { "enabled": true, "names": ["Alex Taylor", "Sam Rivera"] }
```
...
- **State Management**: Zustand 5 (config persisted to `cab-config.json`, UI state to localStorage)
- **Backend**: Tauri 2 (Rust)
- **AI SDKs**: OpenAI SDK v6, Azure Communication Services (calling/chat/identity), Azure Speech SDK, `@microsoft/agents-copilotstudio-client`, `github-copilot-sdk` (Copilot CLI agent)
- **MCP Server**: `rmcp` 2 + `axum` 0.8 (HTTP Streamable transport)
- **Auth**: API key authentication + rate limiting; OAuth tokens in OS credential store (DPAPI)
...
Configuration lives in `cab-config.json` in the app data directory. **OAuth access/refresh tokens are stored in the OS credential store** (Windows Credential Manager / DPAPI) rather than in the WebView localStorage, so they are not left in plaintext readable by page scripts. Other secrets (API keys, access keys, client secrets) live in `cab-config.json`, which is readable by the local OS user — the same threat model as SSH config, browser cookies, or `.env` files.
...
This way the config file can be safely committed, shared, or exported without exposing credentials. (A **Open folder** button in Settings reveals the config directory.)

### Application Hardening

- **Scoped outbound HTTP** — the Tauri HTTP allowlist and CSP `connect-src` enumerate only the specific Azure/Speech/Foundry/login hosts the app uses (no wildcard `https:`).
- **Secret-bearing calls run in Rust** — OAuth token exchanges go through a Rust `http_proxy_request` command (no browser `Origin` header, avoids AADSTS9002326), keeping tokens out of the renderer.
- **XPIA guardrails** — participant captions/chat are wrapped as untrusted content in LLM prompts, intent-detection output is strictly coerced, and the Copilot CLI agent's `shell` tool is denied by default.
- **`https://`-only external links** and validated schemes.

### MCP Server Security

- **Localhost only** — The MCP server binds to `127.0.0.1`, never `0.0.0.0`
- **API key required** — Auto-generated API key must be provided on every request (constant-time comparison)
- **Origin/Host allowlist** — Non-loopback `Origin`/`Host` requests are rejected (anti DNS-rebinding)
- **Rate limiting** — Built-in sliding-window rate limiter (60 requests/minute)
