    ...
    pub async fn respond(
        ...
        let sender = self
            ...
            .ok_or_else(|| {
                tracing::warn!(
                    request_id = request_id,
                    "MCP bridge response arrived without a pending request"
                );
        ...
        sender.send(response).map_err(|_| {
            tracing::warn!(
                request_id = request_id,
                "MCP bridge response receiver dropped before delivery"
            );
...
// `emit` lives on the Emitter trait in Tauri v2
use tauri::Emitter;
